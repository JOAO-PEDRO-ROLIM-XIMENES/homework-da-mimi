...
# histograma
hist(emissoes,
     main = "Emissão do gás",    # título do gráfico
     xlab = "Emissão",           # eixo x
     ylab = "Frequência",        # eixo y
     col = "skyblue",            # cor das barras
     border = "black",           # ajuste entre as barras
     breaks = 15,                # quantidade de intervalos
     cex.main = 1.2,             # Tamanho do título
     cex.lab = 1.2               # Tamanho dos rótulos dos eixos
)

# linha de indicação da média 
abline(v = mean(emissoes), col = "white", lwd = 2)
text(mean(emissoes), 2, "Média", col = "white", pos = 4)
